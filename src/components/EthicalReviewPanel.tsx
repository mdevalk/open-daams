...
const STATUS_LABELS: Record<string, string> = {
  ...
  PENDING: 'In afwachting van toetsing',
  ...
  return (
    <div className="rounded border border-gray-200 bg-white p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-gray-900 text-sm">Ethische toetsing (D6.3 §6.1)</h2>
      ...
      {!editing && (
        <div className="text-sm space-y-1">
          {application.ethicalReviewRequired && (
            <>
              <div className="flex justify-between"><span className="text-gray-500">Toetsingscommissie</span><span>{application.ethicalReviewBody || '—'}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Referentie</span><span>{application.ethicalReviewReference || '—'}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Datum</span><span>{formatDate(application.ethicalReviewDate)}</span></div>
            </>
          )}
          {!application.ethicalReviewRequired && (
            <p className="text-xs text-gray-500">Voor dit project is geen ethische toetsing vereist volgens de aanvrager/behandelaar.</p>
          )}
        </div>
      )}
      ...
      {canManage && editing && (
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-xs text-gray-700">
            ...
            Ethische toetsing is vereist voor dit project
          </label>
          {required && (
            <>
              <div>
                ...
                <select value={status} onChange={e => setStatus(e.target.value as typeof status)}
                  ...
                  <option value="PENDING">In afwachting van toetsing</option>
                  <option value="APPROVED">Goedgekeurd</option>
                  <option value="REJECTED">Afgewezen</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Toetsingscommissie</label>
                <input type="text" value={body} onChange={e => setBody(e.target.value)}
                  className="w-full rounded border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#01689b]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Referentienummer</label>
