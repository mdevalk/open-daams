...
type ApplicationDetail = NonNullable<Awaited<ReturnType<typeof loadApplication>>>;
...
export function CompletenessSection({
  ...
  application: Pick<ApplicationDetail, 'id' | 'status' | 'completenessCheck'>;
  currentUser: CurrentUser;
}) {
  if (application.status !== 'PRE_SCREENING' && !application.completenessCheck) return null;
  return (
    <CompletenessCheckPanel
      applicationId={application.id}
      currentUserId={currentUser.id}
      canManage={application.status === 'PRE_SCREENING' && MANAGE_ROLES.includes(currentUser.role)}
      existing={
...
export function AssessmentSection({
  ...
  application: Pick<ApplicationDetail, 'id' | 'status' | 'assessmentCheck'>;
  currentUser: CurrentUser;
}) {
  if (application.status !== 'PROCESSING' && !application.assessmentCheck) return null;
  return (
    <AssessmentCheckPanel
      applicationId={application.id}
      currentUserId={currentUser.id}
      canManage={application.status === 'PROCESSING' && MANAGE_ROLES.includes(currentUser.role)}
      existing={
...
export function CaseManagementSection({
  ...
  application: Pick<ApplicationDetail, 'type' | 'submittedAt' | 'decisionDeadline' | 'decisionOutcome'> & {
    applicant: Pick<ApplicationDetail['applicant'], 'name'>;
  };
  t: Translator;
  locale: string;
  currentPermit: Pick<ApplicationDetail['dataPermits'][number], 'id' | 'permitNumber' | 'version'> | null;
}) {
  return (
...
type DatasetVariableGroup = {
  sourceDatasetId: string;
  name: string | undefined;
  url: string | null | undefined;
  variables: ApplicationDetail['datasetVariables'];
};

export function DatasetVariablesSection({ groups, t }: { groups: DatasetVariableGroup[]; t: Translator }) {
...
export function ProjectDescriptionSection({
  ...
  application: Pick<
    ...
    | 'title'
    | 'projectLeaderName'
    | 'projectLeaderCountry'
    | 'purposeCategories'
    | 'purposeCategory'
    | 'projectDescription'
    | 'theResearchFocusesOnTheFollowingObjectives'
    | 'theResearchFocusesOnTheFollowingObjectivesOther'
    | 'areaOfResearch'
    | 'areaOfResearchOther'
    | 'descriptionOfTheDataYouWillUse'
    | 'descriptionOfTheProject'
    | 'summaryOfTheProject'
    | 'theNatureOfTheProjectDoesNotLetYouProvideASummaryReason'
  >;
  t: Translator;
}) {
  return (
...
export function ApplicantSection({
  ...
  application: Pick<
    ApplicationDetail,
    'contactPersonOrganisationName' | 'contactPersonOperatorID' | 'applyingForMandatedTasks'
  > & { applicant: Pick<ApplicationDetail['applicant'], 'name' | 'email'> & { dataUser: { name: string } | null } };
  t: Translator;
}) {
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
...
export function BillingSection({
  ...
  billingDetails: ApplicationDetail['billingDetails'];
  t: Translator;
}) {
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-3 text-sm">
        {billingDetails.fullName && (
          <div>
            <dt className="text-gray-500">{t('applicant')}</dt>
            <dd className="font-medium">{billingDetails.fullName}</dd>
          </div>
        )}
        {billingDetails.email && (
          <div>
            <dt className="text-gray-500">{t('contactEmail')}</dt>
            <dd className="font-medium">{billingDetails.email}</dd>
          </div>
        )}
        {billingDetails.organisationName && (
          <div>
            <dt className="text-gray-500">{t('organisation')}</dt>
            <dd className="font-medium">{billingDetails.organisationName}</dd>
          </div>
        )}
        {billingDetails.invoiceType && (
          <div>
            <dt className="text-gray-500">{t('invoiceType')}</dt>
            <dd className="font-medium">{billingDetails.invoiceType}</dd>
          </div>
        )}
        {billingDetails.vatNumber && (
          <div>
            <dt className="text-gray-500">{t('vatNumber')}</dt>
            <dd className="font-medium">{billingDetails.vatNumber}</dd>
          </div>
        )}
        {billingDetails.isProjectFinanciallyCovered !== null && (
          <div>
            <dt className="text-gray-500">{t('financiallyCovered')}</dt>
            <dd className="font-medium">{billingDetails.isProjectFinanciallyCovered ? t('yes') : t('no')}</dd>
          </div>
        )}
        {billingDetails.financingAmountRange && (
          <div>
            <dt className="text-gray-500">{t('financingAmountRange')}</dt>
            <dd className="font-medium">{billingDetails.financingAmountRange}</dd>
          </div>
        )}
      </dl>
    </section>
...
export function LegalBasisSection({
  ...
  application: Pick<
    ...
    | 'summaryOfPlanForUsingTheDataLanguage'
    | 'summaryOfResearchPlanLanguage'
    | 'personResponsibleName'
    | 'personResponsibleSameAsContactPerson'
    | 'personResponsibleJobTitle'
    | 'personResponsibleAffiliation'
    | 'personResearchName'
    | 'personResearchSameAsContactPerson'
    | 'personResearchJobTitle'
    | 'personResearchAffiliation'
    | 'attachments'
  >;
  t: Translator;
}) {
  const section5Attachments = application.attachments.filter((a) => a.field.startsWith('section5.'));
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      {section5Attachments.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2 border-t border-gray-100 pt-3">
          {section5Attachments.map((a) => (
            <a
              key={a.id}
...
export function DatasetsAndPopulationSection({
  application,
  t,
  cohortRows,
}: {
  ...
  >;
  t: Translator;
  cohortRows: ApplicationDetail['studyCohorts'];
}) {
  const section6Attachments = application.attachments.filter((a) => a.field.includes('section6'));
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="mt-4 space-y-3 text-sm">
        ...
        {section6Attachments.length > 0 && (
          <div className="flex flex-wrap gap-2 border-t border-gray-100 pt-3">
            {section6Attachments.map((a) => (
              <a
                key={a.id}
                href={attachmentHref(a)}
                target="_blank"
...
export function DataRequestDetailsSection({
  application,
  t,
}: {
  application: Pick<
    ApplicationDetail,
    'type' | 'ethicalReviewInput' | 'whatIsTheFrequencyOfUpdates' | 'tabulationPlan' | 'tabulationPlans'
  >;
  t: Translator;
}) {
  if (
...
export function OtherDataSection({
  ...
  application: Pick<
    ...
    | 'otherDataDatabases'
    | 'otherDataDatasets'
    | 'otherDataCombinationMethod'
    | 'hasPendingPermitApplications'
    | 'pendingApplicationIssuer'
    | 'pendingApplicationPermitCode'
      ...
      <dl className="space-y-3 text-sm">
        ...
        {application.hasPendingPermitApplications && (
          <div>
            ...
            <dd className="font-medium">
              {[application.pendingApplicationIssuer, application.pendingApplicationPermitCode].filter(Boolean).join(' · ')}
...
type TransferArticleFlags = ([key: string, flag: boolean])[];

export function TransferAndProcessingSection({
  application,
  t,
  transferArticleFlags,
}: {
  application: Pick<
    ApplicationDetail,
    | 'projectStartDate'
    ...
    | 'dataAccessLaterDate'
    | 'dataAccessPeriodInfo'
    | 'dataAccessUpdateFrequency'
    | 'transfersOutsideEuEea'
    | 'transferCountries'
    | 'legalBasisForTransferringTheDataOutsideEU'
    | 'legalBasisForTransferringTheDataOutsideEUOtherOptions'
    | 'safeguardsAreProvidedByReferringGDCP'
    | 'safeguardsAreProvidedByOtherExceptionalLegalBases'
    | 'complyWithDataMinimisationPrincipleNotEUMember'
    | 'lawfulnessOfProcessing'
    | 'lawfulnessLegalBasisOther'
    | 'lawfulForProcessingPersonalData'
    | 'europeanUnionInstitution'
    | 'legalBasisForProcessingCombinedData'
    | 'otherLegalBasisForProcessingCombinedData'
    | 'legalBasisForProcessingApplicationData'
    | 'otherLegalBasisForProcessingApplicationData'
    | 'legalBasisForProcessingCombinedApplicationData'
    | 'otherLegalBasisForProcessingCombinedApplicationData'
    | 'dataProcessingPersonnel'
  >;
  t: Translator;
  transferArticleFlags: TransferArticleFlags;
}) {
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <ul className="list-disc list-inside space-y-0.5">
              {transferArticleFlags.map(([key]) => (
                <li key={key} className="font-medium">
                  {t(`transferArticle.${key}` as Parameters<typeof t>[0])}
                </li>
              ))}
            </ul>
          </div>
...
export function AdditionalInfoSection({
  ...
  application: Pick<ApplicationDetail, 'additionalInformation' | 'attachments'>;
  t: Translator;
}) {
  const section9Attachments = application.attachments.filter((a) => a.field.startsWith('section9.'));
  if (!application.additionalInformation && section9Attachments.length === 0) return null;
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      {section9Attachments.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2 border-t border-gray-100 pt-3">
          {section9Attachments.map((a) => (
            <a
              key={a.id}
              href={attachmentHref(a)}
              target="_blank"
...
export function ConsentSection({
  ...
  application: Pick<
    ApplicationDetail,
    | 'consentAwareProcessingFee'
    | 'consentAwareChargeFee'
    | 'consentAwareInformationCorrect'
    | 'consentNoAccessToUnderlyingData'
    | 'consentAcceptHealthDataBody'
  >;
  t: Translator;
}) {
  if (
...
export default async function ApplicationDetailPage({
  ...
  const datasetVariableGroups: DatasetVariableGroup[] = Array.from(
  ...
  const transferArticleFlags: TransferArticleFlags = (
    [
  ...
  ).filter(([, flag]) => flag) as TransferArticleFlags;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <CompletenessSection application={application} currentUser={currentUser} />
          <AssessmentSection application={application} currentUser={currentUser} />
          <CaseManagementSection application={application} t={t} locale={locale} currentPermit={currentPermit} />
          <DatasetVariablesSection groups={datasetVariableGroups} t={t} />
          <ProjectDescriptionSection application={application} t={t} />
          <ApplicantSection application={application} t={t} />
          <BillingSection billingDetails={application.billingDetails} t={t} />
          <LegalBasisSection application={application} t={t} />
          <DatasetsAndPopulationSection application={application} t={t} cohortRows={cohortRows} />
          <DataRequestDetailsSection application={application} t={t} />
          <OtherDataSection application={application} t={t} />
          <TransferAndProcessingSection application={application} t={t} transferArticleFlags={transferArticleFlags} />
          <AdditionalInfoSection application={application} t={t} />
          <ConsentSection application={application} t={t} />
        ...
        <div className="space-y-4">
          ...
          {application.decisionOutcome === 'POSITIVE' && (
            <ExtractionRequestsPanel
              ...
              canManage={MANAGE_ROLES.includes(currentUser.role)}
            />
          )}
