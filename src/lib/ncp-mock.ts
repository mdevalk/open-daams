/**
 * HealthData@EU NCP query — HDAB-NL test environment, with a mock fallback.
 *
 ...
 * (https://ap.hdab-nl.eu/national-dispatcher/); if `NCP_API_KEY` isn't
 * configured or the call fails, it falls back to a fixed set of realistic
 * sample entries so the retrieval/transmission flow can still be demoed
 * without the test environment being reachable.
 */
 ...
 * either way. Symmetrical to getMockNcpQueue() below, which fakes the
 * inbound leg for the same reason.
 */
...
export async function getMockNcpQueue(): Promise<NcpQueueEntry[]> {
  const headers = ncpHeaders();
  if (headers) {
    try {
      const res = await fetch(`${NCP_BASE_URL}queue`, { headers });
      if (!res.ok) throw new Error(`NCP queue GET returned ${res.status}`);
      return (await res.json()) as NcpQueueEntry[];
    } catch (err) {
      console.warn(
        `[NCP] Queue fetch from ${NCP_BASE_URL}queue failed, falling back to fixture data:`,
        err instanceof Error ? err.message : err,
      );
    }
  }
...
const FIXTURE_QUEUE: NcpQueueEntry[] = [
  {
